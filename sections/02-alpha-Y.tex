...
\item[yóo=]\label{m:yóo=away}
	Directional preverb ‘off, away, out’.
	Usually glossed as ‘off’, ‘away’, or ‘out’ depending on context, with no particular
		abbreviations in common use;
		may sometimes be translated as indefinite ‘somewhere’ or ‘some place’.
	Phonologically more or less identical to the long vowel form
		of the distal deictic element \fm{yú} \~\ \fm{yóo} ‘there (far)’
		but this \fm{yóo=} refers to an indefinite, nonspecific location
		(‘off somewhere’)
		rather than a particular location
		(‘over there’).
	Some cases may be difficult or impossible to distinguish from deictic \fm{yóo}
		as they are structurally identical and can be semantically ambiguous.
	...
	All three of these forms with postpositions do occur,
		but \fm{yóo=} also occurs alone
		in contexts where it is not better analyzed
		as the quotative \X[yóo=quot]{yóo=}
		or the \X[yóo=thus]{yóo=} allomorph of \X{yéi=} ‘thus, so’.

	Originally derived from the distal deictic element
		\fm{yú} \~\ \fm{yóo} ‘there (far)’
		like the similar \X{yux̱=} ‘out, outside’.
	...
	\item	\label{item:yóo=quot-mot}
		...
		\item	\label{item:yóo=quot-mot-de}
			Forms based on the motion derivation
				\motderiv{yóot= / yóox̱= / yóode=}{∅, \fm{-μμL} rep}{off, away}
				with allative \fm{-de} ‘to, toward’.
			...
			\item	\vbform{yóode kei analkélʼ}{prog}[tr, \fm{∅}, mot]{he was off chasing/tracking them}
				orig.\ “(he) was off chasing after (them)”
				\parencite[66.511]{nyman-leer:1993}
					\vbmorph{\gm{yóo}&\gm{-de=}&kei=&a-&na-&lˢⱥ-&\rt[²]{kelʼ}&-μH}
						{off&\·\xx{all}&up&\xx{3>3}&\xx{ncnj}&\xx{xtn}&\rt[²]{undo}&\·\xx{var}}
				\newline
				This has both the \fm{yóot=/yóox̱=/yóode=} motion derivation and the
					\motderiv{kei=}{∅, \fm{-ch} rep}{up} motion derivation.
			\item	\vbform{Gwál de yóode yaa gaxéex}{prog csec}[obj intr, \fm{g}, mot]{maybe having been rising up}
				orig.\ “It was perhaps late afternoon”
				\parencite[122.493]{nyman-leer:1993}
					\vbmorph{\gm{yóo}&\gm{-de=}&ÿaa=&ga-&\rt[¹]{xix}&-μμH&}
						{off&\·\xx{all}&along&\xx{gcnj}&\rt[¹]{fall}&\·\xx{var}&\xx{sub}}
				\newline
				Verbs based on \fm{\rt[¹]{xix}} ‘fall, move through space’ are used
					to describe the movement of the sun or moon
					\parencite[627]{leer:1976}, hence the original translation.
				This progressive consecutive aspect form
					features the \fm{g} conjugation class
					as signalled here by \fm{ga-}
					(consecutive \fm{\xx{cnj}-…-μμH})
					and this with \fm{ÿaa=} ‘along’
					suggests the presence of a secondary aspectual derivation, 
					what \citeauthor{leer:1991} calls “epiaspect”
					\parencite[216–217]{leer:1991}.
				The \fm{ÿaa=} ‘along’ in this context indicates
					the progressive aspect
					of a \fm{∅} or \fm{n} conjugation class verb.
				The combination of the \fm{ga-} conjugation prefix
					and \fm{-μμH} stem variation together
					indicates consecutive aspect (‘having come to pass’).
				The presence of \fm{ga-} rather than the \fm{na-} conjugation prefix
					as expected for progressive aspect
					fits with \citeauthor{leer:1991}’s description of a verb
					shifting from its original conjugation class
					to the \fm{g} conjugation class
					when undergoing secondary aspectual derivation.
				Given all this and the presence of \fm{yóode=} we can conclude
					that this motion verb features
					the \fm{∅} conjugation class motion derivation.
				The presence of \fm{-de} seems to be from
					...
					that would be expected to select \fm{-t}
					whereas the progressive has an imperfective-like meaning
					that would be expected to select \fm{-de}.
			\item	\vbform{dáanaa yóode aléet}{impfv}[tr, \fm{∅}, \fm{-μμH} act]{he’s throwing money away}
				\parencite[227.3220]{story-naish:1973}
					\vbmorph{\gm{yóo}&\gm{-de=}&a-&\rt[²]{lit}&-μμH}
						{off&\·\xx{all}&\xx{3>3}&\rt[²]{scatter}&\·\xx{var}}
				\newline
				...
				Since imperfective aspect entails an incomplete event,
					the presence of the allative \fm{-de} ‘toward’ here
					instead of the punctual \fm{-t} ‘arriving at’
					makes sense because the punctual would imply completion.
			\end{itemize}
		\item	\label{item:yóo=quot-mot-t}
			Forms based on the motion derivation
				\motderiv{yóot= / yóox̱= / yóode=}{∅, \fm{-μμL} rep}{off, away}
				with punctual \fm{-t} ‘at, to, around’.
			See \X{yóot=} for more.
			\begin{itemize}
			\item	\vbform{yóot uwagút}{pfv}[subj intr, \fm{∅}, mot]{he left}
				\parencite[238.407]{dauenhauer-dauenhauer:1987}
					\vbmorph{\gm{yóo}&\gm{-t=}&u-&wa-&\rt[¹]{gut}&-μH}
						{away&\·\xx{pnct}&\xx{pfv}&\xx{stv}&\rt[¹]{go.\xx{sg}}&\·\xx{var}}
			...
			\item	\vbform{x̱ʼaháat aax̱ yóot x̱waakʼwách}{pfv}[tr, \fm{∅}, mot]{I broke the door off from there}
				\parencite[35.317]{story-naish:1973}
					\vbmorph{a&-μx̱&\gm{yóo}&\gm{-t=}&ʷ-&x̱a-&μ-&\rt[²]{kʼwach}&-μH}
						{\xx{3n}&\xx{abl}&off&\·\xx{pnct}&\xx{pfv}&\xx{1sg.s}&\xx{stv}&\rt[²]{break.joint}&\·\xx{var}}
			\item	\vbform{sdoox tóonáx̱ yóot wujitúk}{pfv}[obj intr, \fm{∅}, mot]{it exploded through the stove}
				orig.\ “the stove exploded (that is, the explosion came through the door or lid)”
				\parencite[84.1030]{story-naish:1973}
					\vbmorph{\gm{yóo}&\gm{-t=}&wu-&d-&sh-&i-&\rt[¹]{tuᴴk}&-μH}
						{out&\·\xx{pnct}&\xx{pfv}&\xx{mid}&\xx{pej}&\xx{stv}&\rt[¹]{explode}&\·\xx{var}}
			\item	\vbform{yi tula.aaní yi yadooknáx̱ yóot kawdigán}{pfv}[obj intr, \fm{∅}, mot]{your kindness shines out through your faces}
				\parencite[188.2603]{story-naish:1973}
					\vbmorph{\gm{yóo}&\gm{-t=}&ka-&w-&d-&i-&\rt[¹]{gan}&-μH}
						{out&\·\xx{var}&\xx{hsfc}&\xx{pfv}&\xx{mid}&\xx{stv}&\rt[¹]{burn}&\·\xx{var}}
			\item	\vbform{du jeedáx̱ yóot wuduwasʼélʼ}{pfv}[tr, \fm{∅}, mot]{they tore it out from his hands}
				\parencite[110.41]{dauenhauer-dauenhauer:1987}
					\vbmorph{\gm{yóo}&\gm{-t=}&wu-&du-&wa-&\rt[²]{sʼelʼ}&-μH}
						{out&\·\xx{pnct}&\xx{pfv}&\xx{ind.h.s}&\rt[²]{tear}&\·\xx{var}}
			\item	\vbform{aax̱ yóot awsix̱útʼ}{pfv}[tr, \fm{∅}, mot]{he dragged it out from there}
				\parencite[114.92]{dauenhauer-dauenhauer:1987}
					\vbmorph{a&-μx̱&\gm{yóo}&\gm{-t=}&a-&w-&s-&i-&\rt[²]{x̱utʼ}&-μH}
						{\xx{3n}&\·\xx{abl}&out&\·\xx{pnct}&\xx{3>3}&\xx{pfv}&\xx{xtn}&\xx{stv}&\rt[²]{drag}&\·\xx{var}}
			\item	\vbform{yaakw yóot uwagásʼ}{pfv}[subj intr, \fm{∅}, mot]{the canoe started off/moved over there}
				\parencite[135.1818]{story-naish:1973}
					\vbmorph{\gm{yóo}&\gm{-t=}&u-&wa-&\rt[¹]{gasʼ}&-μH}
						{off&\·\xx{pnct}&\xx{zpfv}&\xx{stv}&\rt[¹]{pole.move}&\·\xx{var}}
				\newline
				The original translation suggests a semantic ambiguity:
					“started off” implies that \fm{yóo} is non-referential
					(not indicating any particular location)
					whereas “over there” implies that \fm{yóo} is referential
					(indicating a particular distal location).
				We can explain the ambiguity by the choice of two motion derivations:
					the \fm{NP-t/x̱/dé} derivation
					with a referential deictic element \fm{yóo} ‘there (far)’
					as the NP within the PP
					or instead the \fm{yóot/yóox̱/yóode} derivation
					with the non-referential \fm{yóo=}.
				The structures of these two would be exactly the same, but the
					meaning of \fm{yóo} in each case would be different,
					presumably reflecting different homophonous lexical items.
			\item	\vbform{xʼúkjaa anax̱ yóot kaawa.úk}{pfv}[obj intr, \fm{∅}, mot]{the steam boiled out through there}
				orig.\ “the steam spouted forth”
				\parencite[206.2881]{story-naish:1973}
					\vbmorph{a&-nax̱&\gm{yóo}&\gm{-t=}&ka-&μʷ-&wa-&\rt[¹]{.uᴴk}&-μH}
						{\xx{3n}&\·\xx{perl}&out&\·\xx{pnct}&\xx{qual}&\xx{pfv}&\xx{stv}&\rt[¹]{boil}&\·\xx{var}}
			\end{itemize}
		\item	\label{item:yóo=quot-mot-x̱}
			Forms based on the motion derivation
				\motderiv{yóot= / yóox̱= / yóode=}{∅, \fm{-μμL} rep}{off, away}
				with pertingent \fm{-x̱} ‘at, of, contacting’.
			...
			\item	\vbform{hoon daakahídi yóox̱ yaa ndahún}{prog}[tr, \fm{∅}, mot]{the store is selling off}
				orig.\ “the store is having a sale”
				\parencite[77.932]{story-naish:1973}
					\vbmorph{\gm{yóo}&\gm{-x̱=}&ÿaa=&n-&da-&\rt[²]{hun}&-μH}
						{off&\·\xx{pert}&along&\xx{ncnj}&\xx{apsv}&\rt[²]{sell}&\·\xx{var}}
			...
			\end{itemize}
		\end{enumerate}
	\item	Forms where \fm{yóo=} appears with a postposition but the verb does not
		...
		\item	Forms with one of the \fm{-t}, \fm{-x̱}, or \fm{-de} postpositions
				that are not from the motion derivation 
				\motderiv{yóot= / yóox̱= / yóode=}{∅, \fm{-μμL} rep}{off, away}
				listed in \#\ref{item:yóo=quot-mot}.
			This excludes cases where \fm{yóo} seems to be referential
				and so probably reflects the distal deictic element
				\fm{yú} \~\ \fm{yóo} ‘there (far away)’
				rather than the non-referential \fm{yóo=} ‘off, away’
				that does not refer to any particular location.
			\begin{enumerate}
			\item	Forms with allative \fm{-de} ‘to, toward’ that are apparently
				not from the motion derivation in \#\ref{item:yóo=quot-mot}. 
				\begin{itemize}
				\item	\vbform{yóode nax̱too.aat}{hort}[subj intr, \fm{n}, mot]{let’s go somewhere else}
					\parencite[202.744]{nyman-leer:1993}
						\vbmorph{\gm{yóo}&\gm{-de=}&na-&x̱-&too-&\rt[¹]{.at}&-μμH}
							{&away&\·\xx{all}&\xx{ncnj}&\xx{mod}&\xx{1pl.s}&\rt[¹]{go.\xx{pl}}&\·\xx{var}}
				...
				\item	\vbform{tle yóode kei aawag̱íxʼ}{pfv}[tr, \fm{∅}, mot]{he tossed it up away}
					orig.\ “(he) threw it away”
					\parencite[68.539]{nyman-leer:1993}
						\vbmorph{\gm{yóo}&\gm{-de=}&kei=&a-&μʷ-&wa-&\rt[²]{g̱ixʼ}&-μH}
							{away&\·\xx{all}&up&\xx{3>3}&\xx{pfv}&\xx{stv}&\rt[²]{toss}&\·\xx{var}}
					\newline
					This form is perfective aspect
						for which we would expect \fm{yóot=}
						with the punctual postposition \fm{-t} ‘to, arriving at’
						but instead we find the allative \fm{-de} ‘to, toward’.
					The \fm{kei=} reflects the motion derivation
						\motderiv{kei=}{∅, \fm{-ch} rep}{up}
						which also assigns \fm{∅} conjugation class.
					It is likely that \fm{yóode=} here
						is an adjunct postposition phrase
						which would be insensitive to conjugation class.
				\end{itemize}
			\item	Forms with punctual \fm{-t} ‘at, to, around’ that are apparently
				not from the motion derivation in \#\ref{item:yóo=quot-mot}.
			\item	Forms with pertingent \fm{-x̱} ‘at, of, contacting’ that are apparently
				not from the motion derivation in \#\ref{item:yóo=quot-mot}.
				\begin{itemize}
				\item	\vbform{du jeeyís yóox̱ sh jikax̱wdináa}{pfv}[tr, \fm{∅}, \fm{-μμH} act]{I made myself go off to work for him}
						orig.\ “I started working for him (that is, set myself to work for him)”
					\parencite[143.1937]{story-naish:1973}
						\vbmorph{\gm{yóo}&\gm{-x̱=}&sh=&ji-&ka-&ʷ-&x̱-&d-&i-&\rt[²]{na}&-μμH}
							{off&\·\xx{pert}&\xx{rflx.o}&hand&\xx{qual}&\xx{pfv}&\xx{1sg.s}&\xx{mid}&\xx{stv}&\rt[²]{tell.go}&\·\xx{var}}
				\end{itemize}
			\end{enumerate}
		\item	Forms with postpositions other than \fm{-t}, \fm{-x̱}, or \fm{-de}.
	...
	\item	Forms where \fm{yóo=} occurs without any postpositions.
		...
		\item	\vbform{Yóo yagútk}{rep impfv}[subj intr, \fm{n}, mot]{he (Kaatsʼ) would go out}
			\parencite[236.381]{dauenhauer-dauenhauer:1987}
				\vbmorph{\gm{yóo=}&ÿa-&\rt[¹]{gut}&-μH&-k}
					{off&\xx{stv}&\rt[¹]{go.\xx{sg}}&\·\xx{var}&\·\xx{rep}}
			\newline
			This seems to be missing the alternating eventuality preverb
				\X[yoo=alt]{yoo=} ‘back and forth, to and fro’
				which would be expected along with stative \X[ÿa-stv]{ÿa-}
				and repetitive \X{-k},
				but the form here is not the predicted \fm{yóo yoo yagútk}.
			We might try to explain \fm{yóo=} as a typo of \fm{yoo=},
				predicting the simpler form \fm{yoo yagútk},
				but the translation ‘go out’ supports \fm{yóo=} ‘off, away’
				so it does not seem to be a typo.
			It could instead be that \fm{yóo=} has blocked \fm{yoo=},
				perhaps due to haplology (avoidance of duplication),
				or it could be that the speaker mistakenly dropped \fm{yoo=}.
		\end{itemize}
