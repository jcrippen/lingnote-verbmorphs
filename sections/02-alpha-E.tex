...
\item[éeg̱i=]\label{m:éeg̱i=}
	...
	Derived from the noun \fm{éeḵ} \~\ \fm{éiḵ} ‘beach’
		with the special locative postposition allomorph \fm{-í} \~\ \fm{-i} ‘at’
		(instead of the regular allomorphs \fm{-xʼ} and \fm{-μ});
		see \X{éig̱i=} for more details.
...
\item[éig̱i=]\label{m:éig̱i=}
	...
	This locative allomorph is unique in that it only occurs with a handful of preverbs:
		\begin{inlinelist}
		\item	\fm{dáag̱i=} ‘inland’
		\item	\fm{héeni=} ‘in water’
		\item	\fm{gáani=} ‘outside’
		\item	\fm{gági=} ‘into open’
		\item	\fm{neilí=} ‘inside’
		\item	\fm{x̱áni=} ‘near’;
		\end{inlinelist}
	otherwise the locative postposition occurs as either \fm{-xʼ} or \fm{-μ} (after /\ipa{CV}/).
