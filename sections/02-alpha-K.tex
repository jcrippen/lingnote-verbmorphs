...
\item[kéi=]\label{m:kéi=}
	Variant form of direction preverb \X{kei=} ‘up’.
...
\item[kux=]\label{m:kux=}
	Directional preverb meaning ‘aground, shallows’.
	Derived from the root \rt{\fm{kux}} ‘dry’ as in
		\fm{shaawakúx} ‘she become thirsty’,
		\fm{shakoox} ‘thirst’,
		\fm{kawlikúx} ‘it drained out, went dry’,
		\fm{akawsikúx} ‘she bailed it’,
		and \fm{kakúxaa} ‘bailer’.
	Reported by \citeauthor{leer:1991}
		as a motion derivation
		\motderiv{kux= / kuxx̱= / kúxde=}{∅, \fm{-μμL} rep}{moving aground, into shallow water}
		\parencite[133, 137, 138 fn.\ 53, 299]{leer:1991}.
	But only the \fm{kux=} form is attested, and that only once in a single sentence from
		\fm{Seidayaa} Elizabeth Nyman (see below).
	It is unclear if the \fm{kuxx̱=} and \fm{kúxde=} forms actually occur or if these were
		extrapolated by \citeauthor{leer:1991} from the one attestation.
	\begin{itemize}
	\item	\vbform{kux wuduwashát wé chʼaḵúx̱ ḵu.a}{pfv}[tr, \fm{∅}, mot]{they pulled the canoe aground}
		\parencite[62.440]{nyman-leer:1993}
			\vbmorph{\gm{kux=}&wu-&du-&wa-&\rt[²]{shaᴴt}&-μH}
				{aground=&\xx{pfv}&\xx{ind.h.s}&\xx{stv}&\rt[²]{grab}&\·\xx{var}}
	\end{itemize}

\item[kuxx̱=]\label{m:kuxx̱=}
	Allomorph of directional preverb \X{kux=} ‘aground, shallows’
		with the pertingent postposition \fm{-x̱} ‘at, of, concerning’.
	Reported by \textcite[299]{leer:1991} but not attested.
	See \X{kux=} for discussion.

\item[kúxde=]\label{m:kúxde=}
	Allomorph of directional preverb \X{kux=} ‘aground, shallows’
		with the allative postposition \fm{-de} ‘to, toward’.
	Reported by \textcite[299]{leer:1991} but not attested.
	See \X{kux=} for discussion.
