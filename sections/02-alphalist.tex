	...
	\item	unclear meaning but probably deprivative in several nouns and adjectives
		...
		\item	\fm{kax̱duḵéisʼáḵw} \~\ \fm{katḵéisʼáḵw} ‘quilt’
			from \fm{\rt{ḵa}} ‘stitch, sew’
				\vbmorph{ka-&x̱-&du-&\rt[²]{ḵa}&-μμᵉH&-sʼ&\gm{-áḵw}}
					{\xx{hsfc}&\xx{g̱cnj}&\xx{ind.h.s}&\rt[²]{stitch}&\·\xx{var}&\·\xx{rep}&\·\xx{dprv}}
				 or \vbmorph{ka-&d-&\rt[²]{ḵa}&-μμᵉH&-sʼ&\gm{-áḵw}}
				 	{\xx{hsfc}&\xx{mid}&\rt[²]{stitch}&\·\xx{var}&\·\xx{rep}&\·\xx{dprv}}
...
\item[-sʼ]\label{m:-sʼ}
	...
	occurs instead of the conjugation class determined repetitive suffix
		for some verbs, but other verbs have both
		the conjugation class determined suffix
		and this \fm{-sʼ} suffix
	...
	\item	repetitive suffix with a wide variety of roots,
		...
		\item	\fm{–ḵéisʼ} from \fm{\rt{ḵa}} ‘stitch, sew’ in
			...
			\end{inlinelist}
		\end{itemize}
		there are too many roots that occur with \fm{-sʼ} to give examples of every one;
		the following list \parencite[from][533]{crippen:2019} is extensive but may not be complete:
		\begin{inlinelist}
		...
		\end{inlinelist}
	\item	frozen in a few roots with complex codas
	\item	possibly frozen in some CVC roots with coda /\ipa{sʼ}/
	\item	potentially identifiable in a few nouns with ablaut /\ipa{e}/
	\item	potentially identifiable in some nouns without ablaut
	\end{enumerate}

%		\begin{itemize}
%		\item	\fm{kadásʼ} ‘hail’
%			possibly from \fm{\rt{tats}} \~\ \fm{\rt{dats}} ‘knock berries off branch’
%			or perhaps \fm{\rt{daᴴn}} ‘snow fall heavy; dust’
%		\item	\fm{x̱ʼásʼ} ‘jaw, mandible’
%		\end{itemize}	
