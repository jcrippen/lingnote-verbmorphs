...
\item[-kwt]\label{m:-kwt}
	...
	\end{allolist}
	\begin{enumerate}
	\item	repetitive suffix combination in multipositional repetitive
		state imperfectives, indicating entities distributed at multiple
		positions along some path in space
...
\item[-t]\label{m:-t}
	...
	\item	repetitive suffix with any verb that denotes an event involving
		...
		\item	roots attested with ictive \fm{-t} in a repetitive imperfective form
			...
			\end{inlinelist}
		\end{itemize}
	\item	repetitive suffix in some repetitive state imperfectives;
		meaning is unclear but seems to be like ‘occurs easily, has tendency’
			as with tendency states with \X{-k} \~\ \X{-kw};
		may be equivalent to some cases of \X{-kt} \~\ \X{-kwt} (see below)
			but with CVC roots
		\begin{itemize}
		\item	\fm{–níkt} ‘tattle’
	...
	\item	unclear meaning with two verb roots of unknown meaning
		...
		\item	\fm{–shéesht} ‘squeeze out’
			...
			\parencites[10/88]{leer:1973}[552]{leer:1976}
				\vbmorph{a-&w-&lˢ-&i-&\rt{shish}&-μμH&-t}
