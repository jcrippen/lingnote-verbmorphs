...
\item[-sʼ]\label{m:-sʼ}
	...
	for some verbs \fm{-sʼ} occurs instead of
		the conjugation class determined repetitive suffix
		(\fm{-ch}, \fm{-k}, \fm{-x̱})
		but other verbs have both
		the conjugation class determined suffix
		and the \fm{-sʼ} suffix;
	...
	the repetitive suffix \X{-lʼ} is probably historically related,
		similar to the connection between \X{s-} and \X{l-}
	\newline
	allomorphs:
	\begin{allolist}
	\item[-sʼ]	basic form
	...
	\item	repetitive suffix with a wide variety of roots,
		...
		\item	\fm{–ḵéisʼ} from \fm{\rt{ḵa}} ‘stitch, sew’ in
			...
			\end{inlinelist}
		\end{itemize}
		the following list of roots known to occur with \fm{-sʼ} 
			\parencite[from][533]{crippen:2019}
			is extensive but may not be complete:
		\begin{inlinelist}
...
\item[-tʼ]\label{m:-tʼ}
	...
	for some verbs \fm{-tʼ} occurs instead of
		...
		but other verbs have both
		the conjugation class determined suffix
		and the \fm{-tʼ} suffix;
	this suffix appears to be productive,
		apparently involving the destruction of an entity,
		but with some unclear exceptions;
	possible historical relationship with \X{-lʼ}, \X{-sʼ}, and \X{-xʼ}
		since all of these are ejective consonants
	\begin{enumerate}
	\item	repetitive suffix with a variety of roots,
		appearing in a repetitive imperfective form
		or in forms derived from the repetitive imperfective
		(secondary aspectual derivations, Leer’s “epiaspect”)
		\begin{itemize}
		\item	\fm{–gántʼ} ‘burn plural’
			...
			\end{itemize}
		\item	\fm{–héitʼ} ‘erase plural’
		\item	\fm{–kélʼtʼ} ‘become ash’
		\item	\fm{–kweitʼ} ‘get to know plural’
		\item	\fm{–láxwtʼ} ‘plural starve to death’
		...
		\item	\fm{–xóoshtʼ} ‘singe, scorch’
		\end{itemize}
	\item	in some complex nouns
		\begin{itemize}
		\item	\fm{daakakeiltʼí} ‘husk, chaff, dried remains of berry’
		\item	\fm{jaḵtawteetʼí} ‘assassin, secret murderer’
		\item	\fm{kayéx̱tʼi} ‘wood shavings’
		\item	\fm{kakúshtʼi} ‘pitch blister on tree’
		\item	\fm{ḵʼalukakúltʼi} ‘slit, eyehole in thong end’
		\item	\fm{xʼaan kakúshtʼi} ‘cottonwood fluff with seeds’
		\item	\fm{ḵuyasaḵéitʼi} ‘judge, commissioner’
		\item	\fm{at yasaḵéitʼi} ‘lawyer, judge’
		\end{itemize}
	\item	possibly analyzable in some CVCC nouns with coda /\ipa{tʼ}/
		\begin{itemize}
		\item	\fm{íx̱tʼ} ‘shaman, medicine man, Indian doctor’
			from unknown \fm{\rt{.ix̱}}
		\item	\fm{kélʼtʼ} ‘ash’
			also \fm{kéilʼ} ‘ash’
			from \fm{\rt{kelʼ}} ‘take apart, untie; burn to ash’,
			compare \fm{\rt{kelʼ}} ‘run away’
			\newline
			also occurs in:
			\begin{itemize}
			\item	\fm{dleit kakélʼtʼ} ‘light snow’
			\item	\fm{sʼax̱kélʼtʼi} ‘fine powder’
			\item	\fm{sʼeeḵ x̱ʼakélʼtʼi} ‘cigarette ash’
			\end{itemize}
		\item	\fm{xóoshtʼ} ‘singed, scorched matter’
			from \fm{\rt{xush}} ‘singe, scorch’,
			compare \fm{\rt{xuts}} ‘char’
		\item	\fm{sʼáxtʼ} ‘devils club’
			from unknown \fm{\rt{sʼax}},
			compare \fm{sʼáx} ‘starfish’
		\item	\fm{shakalḵʼíshtʼ} ‘deer with four-point horns’
			from \fm{\rt{ḵʼish}} \~\ \fm{\rt{ḵʼesh}} ‘swat puck, ball’
		\end{itemize}
	\item	possibly part of two nouns ending with \fm{…tʼu}
		\begin{itemize}
		\item	\fm{kéitʼu} ‘pick, pickaxe’ 
			...
			compare \fm{káatʼ} ‘digging stick’,
			Haida \fm{kitʼuu} ‘harpoon, seafood spear’ \parencite[1062]{enrico:2005}
		\item	\fm{x̱éitʼu} ‘white film on tongue’
			from unkonwn \fm{\rt{x̱e}} or \fm{\rt{x̱etʼ}};
			compare \fm{x̱aatlʼáḵw} ‘mouth ulcer’,
			\fm{x̱aatlʼ} ‘freshwater grass’,
			\fm{x̱éetʼ} ‘giant clam’
		\end{itemize}
	\end{enumerate}
