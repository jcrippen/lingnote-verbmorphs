...
\item[-ch]\label{m:-ch}
	...
	occurs in several different contexts:
		...
		combined with other repetitive or derivational suffixes (\ref{item:-ch-combo}),
	...
	\item	\label{item:-ch-oddrep}
		...
		\item	\label{item:-ch-oddrep-event}
			event imperfectives with \fm{-ch};
			\citeauthor{leer:1991} considers these to be “primary imperfectives” 
				(so not repetitive) despite the presence of the repetitive
				suffix \fm{-ch} \parencite[245]{leer:1991}
				and often represents them as “A-ch” \parencite[iii]{leer:1978b}
				with “A” for ‘activity’;
				they seem to have repetitive meanings so it is simpler to treat them
				as irregular repetitive imperfective forms
			\begin{itemize}
			\item	\fm{–léich}
		...
		\item	\label{item:-ch-oddrep-state}
			state imperfectives with \X[i-stv]{i-} \~\ \X[ÿa-stv]{ÿa-}
			and \fm{-ch}; no regular examples are attested
	...
	\item	\label{item:-ch-roots}
		...
		\item	\label{item:-ch-roots-CVCC}
			...
			\item	\fm{–núkch} ‘be helpless, undependable’
				perhaps from \fm{\rt[¹]{nuk}} ‘singular sit’ in
				\newline
