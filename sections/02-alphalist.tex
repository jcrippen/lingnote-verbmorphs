...
\item[-álʼ]\label{m:-álʼ}
	...
	but with \fm{…néegwalʼ} ‘paint’ the \fm{-álʼ} occurs in all forms
		and so forms a frozen disyllabic stem;
	the order of \fm{-álʼ} versus \X{-k} \~\ \X{-kw} is interesting
		with \fm{-álʼ-k} in \fm{yoo ayanéegwálʼk} as inflection
		but with \fm{-kw-álʼ} as derivation in nouns
	\begin{itemize}
...
\item[ash=]\label{m:ash=}
	...
	probably derived from third person pronoun \fm{á} + reflexive pronoun \fm{sh}
		or ergative \fm{-ch};
		see also \X{ach=} used instead of \fm{ash=} in some verbs;
	\newline
	allomorphs:
	\begin{allolist}
	\item[ash=]	basic form
