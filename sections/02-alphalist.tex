...
\item[-áḵw]\label{m:-áḵw}
	...
	\item	\fm{–xʼwánjáḵw} ‘remove boots’
		(\fm{\rt{xʼwan}} ‘boot’ + repetitive \X{-ch})
	\item	\fm{–x̱aaÿáḵw} ‘shed, lose hair, go bald’
		(\fm{\rt{x̱aw}} ‘hair’ with unexpected \fm{w} → \fm{ÿ})
	\end{itemize}
	the \fm{-áḵw} form of deprivative \fm{-ḵ} \~\ \fm{-ḵw}
		is also attested in the following verb stems
	...
	\item	\fm{–.éiÿáḵw} ‘have injured limb’
		(perhaps \fm{\rt{.ek}} ‘weak, paralyzed’)
	\item	\fm{–ÿaax̱áḵw} ‘plan, intend’
		(perhaps \fm{\rt{ÿaḵw}} ‘bequeath’)
	\item	\fm{–tlʼeitáḵw} ‘pure, honest’
		(likely \fm{\rt{tlʼeᴴn}} ‘impure, unclean’)
	\item	\fm{–x̱oonáḵw} ‘drowned’
		(\fm{\rt{x̱un}} ‘family’)
	\item	\fm{–chʼéeÿáḵw} ‘slow, late’
		(root unknown, \fm[*]{\rt{chʼi}}, \fm[*]{\rt{chʼiÿ}})	
	\end{itemize}
	the \fm{-áḵw} form of the deprivative also appears in the stem
		...
		and \fm{-áḵw};
	the \fm{-áḵw} suffix and its delabialized form \fm{-áḵ}
		also occur in several nouns:
	\begin{itemize}
	\item	\fm{gaawaḵ} ‘saskatoonberry’
	...
	\item	\fm{shax̱dáḵw} ‘legendary shark’
	\item	\fm{tʼaawáḵ} ‘Canada goose’
	\item	\fm{Tlʼanaxéedáḵw} ‘Property/Wealth Woman’
	\item	\fm{x̱áatlʼáḵw} ‘mouth ulcer’
		which also occurs in
		\fm{washtux̱áatlʼáḵw} ‘cheek ulcer, cold sore’
	\end{itemize}
	finally, \fm{-áḵw} might be identified as delabialized \fm{-áḵ}
		in the root \fm{\rt{naḵ}} ‘abandon, leave behind’
		which occurs as a verb, relational noun, and postposition
...
\item[-chʼálʼ]\label{m:-chʼálʼ}
	...
	compare the stem \fm{–ḵéilʼútʼ} ‘lick steam’ with \X{-lʼútʼ}
	\begin{itemize}
	\item	\fm{ḵéichʼálʼ} \~\ \fm{ḵéechʼálʼ} ‘seam’
...
\item[-ḵ]\label{m:-ḵ-dprv}
	...
	\end{allolist}
	contrast optative/prohibitive \X[-ḵ-optphib]{-ḵ} which is accompanied by a particle;
	both the deprivative \fm{-ḵ} and optative/prohibitive \fm{-ḵ} are related to 
...
\item[-lʼ]\label{m:-lʼ}
	...
	\item	\fm{gúnlʼ} ‘burl, lump’
		...
		the noun is also the basis of the derived verb
			\vbform{kashigúnlʼ}{impfv}[obj intr, conj?, state]{it has a burl}
	\item	\fm{g̱úḵlʼ} ‘swan’
...
\item[-sʼ]\label{m:-sʼ}
	repetitive suffix, perhaps indicating repetition in a sequence or line
	\newline
	allomorphs:
	\begin{allolist}
	\item[-sʼ]	basic form
