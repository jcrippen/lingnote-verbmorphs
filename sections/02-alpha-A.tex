...
\item[áa=]\label{m:áa=}
	...
	Arguably this is an independent phrase rather than a preverb, but the \X[-μ-loc]{-μ} allomorph
		of the locative postposition is only permitted immediately preceding the verb word
		and so this \fm{áa} = \fm{á-μ} is positionally restricted like other preverbs.
	This reflects a more general process of evolution of indepdendent adverbs
		and postposition phrases into preverbs when they occur in front of the verb word.
	\begin{enumerate}
	\item	\label{item:áa-plain}
		...
		\item	\vbform{x̱áat áa ashax̱íshdin}{past impfv}[tr, \fm{∅}/\fm{n}, \fm{-t} rep]{he was clubbing fish there}
			\parencite[50.547]{story-naish:1973}
				\vbmorph{x̱áat&\gm{á}&\gm{-μ=}&a-&sha-&\rt[²]{x̱ish}&-μH&-t&-in}
					{fish&\xx{3n}&\·\xx{loc}&\xx{3>3}&head&\rt[²]{club}&\·\xx{var}&\·\xx{rep}&\·\xx{past}}
		\end{itemize}
...
\item[-a]\label{m:-a}
	Allomorph of the instrument suffix \X{-aa} \~\ \X{-áa}.
...
\item[-aa]\label{m:-aa}
	Instrument suffix where the verb denotes an instrument used for
		the event usually described by the verb.
