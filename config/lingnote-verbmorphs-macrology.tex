...
%%
%% Tried using tabularray (tblr) but it was painfully slow with a large document.
%% Instead this solution uses TikZ matrices. But using & in an argument won’t work
%% because TikZ can't make the ampersand active as explained here:
%%   https://tex.stackexchange.com/q/1111
%% The fix is to rescan the input as detailed here:
%%   https://tex.stackexchange.com/a/619983
%% Although this is slow, it's still much faster than tabularray. The solution
%% using LaTeX3 doesn't work but I don't know why and didn't try to figure it out.
\ProvideDocumentCommand \vbmorph {m m}
	{\lindent{with~}%
...
%% Hyperref crossreference macro.
\ProvideDocumentCommand \X {o m}
	% 1: label
	% 2: text
	{\textcolor{blue}{\IfNoValueOrEmptyTF{#1}{\hyperref[m:#2]{#2}}{\hyperref[m:#1]{#2}}}}
